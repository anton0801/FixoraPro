...
struct FixoraProWebView: View {
    ...
    private func initialize() {
        let temp = UserDefaults.standard.string(forKey: CrateKeys.pushTransientURL)
        let stored = UserDefaults.standard.string(forKey: CrateKeys.spot) ?? ""
        targetURL = temp ?? stored
        isActive = true
        if temp != nil { UserDefaults.standard.removeObject(forKey: CrateKeys.pushTransientURL) }
    }
    
    private func reload() {
        if let temp = UserDefaults.standard.string(forKey: CrateKeys.pushTransientURL), !temp.isEmpty {
            isActive = false
            targetURL = temp
